<template>
        <nav class='navbar' id='navbar'>
            <div class='navbar-container' :class="{ fixed: belowRange }">
        ...
        methods: {
            ...
            handleScroll(){
                if (document.querySelector('#navbar').getBoundingClientRect().top <= 0){
                    document.querySelector('#navbar .navbar-container').classList.add('fixed')
                } else {
                    document.querySelector('#navbar .navbar-container').classList.remove('fixed')
                }
                this.checkComponent()
                console.log(this.belowRange, document.querySelector('#navbar').getBoundingClientRect().top <= 0)
            },
            checkComponent(){
                let names = ['contact', 'blogs', 'about', 'projects', 'home']
                let selected = ''
                names.map(name => {
                    let pixels = 1
                    name == 'projects'
